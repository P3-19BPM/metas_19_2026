...
echo Criando tarefas no boot (ONSTART)...
schtasks /Create /TN "Metas19_API_28100" /SC ONSTART /RU SYSTEM /RL HIGHEST /TR "\"%WSCRIPT%\" \"\"%API_VBS%\"\"" /F
if errorlevel 1 (
  echo [ERROR] Falha ao criar tarefa Metas19_API_28100
  exit /b 1
)

schtasks /Create /TN "Metas19_Agent" /SC ONSTART /RU SYSTEM /RL HIGHEST /TR "\"%WSCRIPT%\" \"\"%AGENT_VBS%\"\"" /F
if errorlevel 1 (
...
echo   schtasks /Run /TN "Metas19_API_28100"
echo   schtasks /Run /TN "Metas19_Agent"
echo Logs:
